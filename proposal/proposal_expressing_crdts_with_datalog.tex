...
\begin{figure*}
	...
	\begin{tikzpicture}
		...
		local-replica/.style={replica},
		remote-replica/.style={replica,scale=0.66},
		label/.style={font=\bfseries},
		rel/.style={
		->, >={Stealth[round]},bend left,},
		}

		\node[local-replica] (local) {\replica};
		\node[label] (local-label) [above=0pt of local] {Local Replica};

		\node[remote-replica] (remote3) [right=of local,yshift=+15pt,xshift=+15pt] {\replica};
		\node[remote-replica] (remote2) [right=of local,yshift=+10pt,xshift=+10pt] {\replica};
		\node[remote-replica] (remote1) [right=of local,yshift=+5pt,xshift=+5pt] {\replica};
		\node[remote-replica] (remote0) [right=of local] {\replica};
	...
	\caption{
		...
		and are distributed and forwarded by the application layer without
		any further processing.
		\deltaO{} is the output delta of all Datalog queries and is based on
