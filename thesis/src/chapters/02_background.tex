...
If \(x_1, \ldots x_m\) are the variables of the rule's body, this can be read as
``for all \(x_1, \ldots, x_m\) it holds that \(a_1\) and \ldots\ and \(a_n\) imply \(r\)''
and expressed\footnotemark{} mathematically as:

\footnotetext{
	Not every atom or head has to reference all variables.
}
...
\footnotetext{
	...
	I think that CRDTs are better characterized by their property that diverging
...
One advantage of DBSP is its modular theory:
...
to express \acp{CRDT} in my use case.
...
\footnotetext{
	...
	commutative semiring, DBSP names them \zsets{}.
}

DBSP calls a query plan a \emph{circuit} and it can be assembled from the
available operators on \emph{streams}.
Streams carry \zsets{} which can represent both data as well as changes to data.
In the former case, their \zweight{} cannot be negative and a positive \zweight{}
indicates the multiplicity of a tuple in the stream.
In the latter case, a positive \zweight{} indicates an insertion
and a negative \zweight{} indicates a deletion of a tuple.
