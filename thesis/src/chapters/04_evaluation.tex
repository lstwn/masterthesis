...
It does not include the time it takes to parse the Datalog query nor to set up
the DBSP circuit, as the near-real-time setting simulates the situation
in which the application has already started up and is running.

\input{figures/bench_kv_store_nrt.tex}

\ref{fig:bench-kv-stores-nrt} shows benchmarks for the key-value stores.
The \emph{delta diameter} \(\in \{20, 40, 60, 80, 100\}\) is the diameter of the
updates, again viewed as a directed graph, and generated as in the hydration
setting.
The benchmark reveals some interesting patterns.
For the key-value store without causal broadcast
from \ref{code:mvr-store-datalog-dialect}, the base diameter has no impact on
the performance.
Varying the delta diameter has only a small impact on the
performance (\ref{fig:bench-kv-store-nrt-nocb}).
Regardless, in any case the time to process the updates is less than a quarter
of a millisecond.
However, the key-value store with causal broadcast
from \ref{code:mvr-crdt-datalog-dialect} also shows only a small impact from
varying the delta diameter.
...
% TODO: Text list results
% TODO: Mention the primitive nature of the benchmarks
