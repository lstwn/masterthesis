\newcommand{\replica}{
	\begin{tikzpicture}
		...
		\draw[rel] (user) to[bend left] node[midway,near start,auto=left,yshift=-7pt]{issues \deltaI{local}} (app);
		\draw[rel] (app) to[bend left] node[auto,sloped]{sees \deltaO{}} (user);
		...
		\draw[rel] (db)
		to[bend right=45, auto=right]
		node[midway, near start]{provides \deltaO{}}
		(app);
	\end{tikzpicture}
}
